...
// ********** 예약관리 관련 메서드 *************
	...
	public List getBookingList(int start, int end) {
		...
		try {
			conn = getConnection();
			String sql = "select booking_num, hotel_name, name, booking_time, check_in, user_name, user_phone, pet_name, requests, booking_status, payment, pet_num, r from (select rownum r, booking_num, hotel_name, name, booking_time, check_in, user_name, user_phone, pet_name, requests, booking_status, payment, pet_num from (select B.booking_num, M.hotel_name, R.name, B.Booking_Time, B.check_in, B.user_name, B.user_phone, P.pet_name, B.requests, B.booking_status, B.payment, P.pet_num from Booking B, room R, member M, pet P where R.room_num = B.room_num AND R.id = M.id AND P.pet_num = B.pet_num order by B.booking_status desc, B.booking_time desc))where r >= ? and r <= ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
			...
			if(rs.next()) {
				...
				do {
					...
					hotel.setBooking_status(rs.getInt("booking_status"));
...
// ********** 사업자 관리 관련 메서드 *************
		...
		public void deleteMemberBooking(String id) {
			PreparedStatement pstmt = null;
			Connection conn = null;
			ResultSet rs = null; // 오라클은 update 사용시 두개 이상의 테이블 참조가 안됌. 따라서 하나하나 넣어서 삭제할 예정
			...
			try {
				conn = getConnection();
				String sql = "select room_num from room where id = ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, id);
				...
				if(rs.next()) {
					...
					for(int i = 0; i < roomNum.size(); i++) {
						room = (RoomDTO)roomNum.get(i);
						sql = "update booking set booking_status = 1 where booking_status = 2 AND room_num = ?";
						pstmt = conn.prepareStatement(sql);
