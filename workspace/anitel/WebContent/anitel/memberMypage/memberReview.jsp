					      ...
					      <li class="table-row">
						      ...
						      <div class="col col-2" style="flex-basis: 45%;">
						      	<%if(sel != null && search != null){ %>
						      		<a class="list_subject" href="../board/content.jsp?board_num=<%= article.getBoard_num()%>&pageNum=<%=pageNum%>&categ=<%=categ%>&sel=<%=sel%>&search=<%=search%>"><%=article.getSubject() %></a>
						      	<%}else{ %>
						      		<a class="list_subject" href="../board/content.jsp?board_num=<%= article.getBoard_num()%>&pageNum=<%=pageNum%>&categ=<%=categ%>"><%=article.getSubject() %></a>
						      	<%} %>
					      	</div>
...
<%		if(count>0){
			...
			if(sel != null && search != null){
				...
				if(startPage > pageBlock){ %>
					<a href="memberReview.jsp?pageNum=<%= startPage-pageBlock %>&sel=<%=sel%>&search=<%=search%>&categ=<%=categ%>&amp;reg_num=<%=member.getReg_num()%>" class="pageNums"> &lt; </a>
<%				}
				// 페이지 번호
				for(int i = startPage; i <= endPage; i++){ %>
					<a href="memberReview.jsp?pageNum=<%=i%>&sel=<%=sel%>&search=<%=search%>&categ=<%=categ%>&amp;reg_num=<%=member.getReg_num()%>" class="pageNums"> &nbsp; [<%= i %>] &nbsp; </a>
<%				}
...
<%				}
				...
				// 왼쪽 꺾쇠 : startPage가 pageBlock(5)보다 크면 생성
					if(startPage > pageBlock){ %>
						<a href="memberReview.jsp?pageNum=<%= startPage-pageBlock %>&categ=<%=categ%>&amp;reg_num=<%=member.getReg_num()%>" class="pageNums"> &lt; </a>
<%					}
				// 페이지 번호
					for(int i = startPage; i <= endPage; i++){ %>
						<a href="memberReview.jsp?pageNum=<%=i%>&categ=<%=categ%>&amp;reg_num=<%=member.getReg_num()%>" class="pageNums"> &nbsp; [<%= i %>] &nbsp; </a>
<%					}
				// 오른쪽 꺾쇠 : 전체 페이지 개수(pageCount)가 endPage(현재 보는 페이지에서의 마지막 번호) 보다 크면
				if(endPage < pageCount){%>
					<a href="memberReview.jsp?pageNum=<%=startPage + pageBlock%>&categ=<%=categ%>&amp;reg_num=<%=member.getReg_num()%>" class="pageNums"> &gt; </a>
		...
		<%-- 아이디 , 글제목 으로 검색 --%>
			<form action="memberReview.jsp"> 
				...
				<div class="search_wrap">
					...
					<input class="btn" type="button" value="원래대로" onclick="window.location='memberReview.jsp'"/>
				</div>
