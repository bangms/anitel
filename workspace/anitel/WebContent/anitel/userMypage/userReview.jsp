	...
	<div id="container">
		...
		<div id="section" style="padding-left: 15%; margin-left: 40px;">
					...
					<li class="table-row">
						...
						<div class="col col-2" style="flex-basis: 30%;">
							<a class="list_subject"
								href="../anitel/board/content.jsp?board_num=<%= article.getBoard_num()%>&pageNum=<%=pageNum%>&categ=<%=categ%>"><%=article.getSubject() %></a>
						</div>
						...
						<div class="col col-5" style="flex-basis: 15%;"><%= article.getReadcount()%></div>
					</li>
					<%		} 
						}%>
				</ul>
			...
			if(endPage > pageCount){
				...
				<a
					href="userReview.jsp?pageNum=<%= startPage-pageBlock %>&categ=<%=categ%>"
					class="pageNums"> &lt; </a>
				<%					}
				// 페이지 번호
					for(int i = startPage; i <= endPage; i++){ %>
				<a href="userReview.jsp?pageNum=<%=i%>&categ=<%=categ%>" class="pageNums">
					&nbsp; [<%= i %>] &nbsp;
				</a>
				<%					}
